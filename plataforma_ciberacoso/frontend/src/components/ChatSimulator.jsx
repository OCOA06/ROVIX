  ...
  */
  ...
  return (
    <div className="w-full h-full flex flex-col overflow-hidden max-w-7xl mx-auto">
      ...
      <button 
        onClick={onBack} 
        className="flex items-center text-ods-brown-medium hover:text-ods-brown-dark mb-6 font-semibold transition-colors flex-shrink-0 cursor-pointer"
      >
      ...
      <div className="flex-1 flex gap-6 items-start min-h-0 overflow-hidden w-full">
        ...
        <div className="flex flex-col items-center flex-shrink-0 h-full overflow-hidden">
          <p className="text-xs font-bold text-ods-brown-medium uppercase tracking-wider mb-3 flex items-center gap-1.5 flex-shrink-0 select-none">
            <span className="text-ods-orange animate-pulse">📱</span> Toca un mensaje para analizarlo
          </p>
          
          {/* Estructura del Dispositivo Móvil en Marrón Tierra Oscuro */}
          <div
            className="w-[320px] border-[12px] border-ods-brown-dark rounded-[2.8rem] bg-ods-beige-light relative overflow-hidden shadow-2xl flex flex-col flex-1"
            style={{ boxShadow: '0 25px 60px rgba(62,42,31,0.25), inset 0 0 0 2px rgba(255,255,255,0.1)' }}
          >
            {/* Notch superior simulador */}
            <div className="absolute top-0 inset-x-0 flex justify-center z-10">
              <div className="w-28 h-6 bg-ods-brown-dark rounded-b-2xl" />
            </div>

            {/* Cabecera del chat simulado */}
            <div className="bg-gradient-to-r from-ods-brown-dark to-ods-brown-medium pt-8 pb-3 px-4 flex items-center gap-2">
              <div className="w-7 h-7 rounded-full bg-white/20 flex items-center justify-center">
                <Shield size={14} className="text-white" />
              </div>
              <div>
                <div className="text-white text-xs font-bold">Chat de práctica</div>
                <div className="text-ods-beige/70 text-[10px]">Mensajes de ejemplo</div>
              </div>
            </div>

            {/* Lista Desplazable de Mensajes de Simulación */}
            <div className="flex-1 overflow-y-auto p-3 space-y-2 bg-gradient-to-b from-ods-beige to-ods-beige-light/30">
                    ...
                    <div className="relative">
                      ...
                      <div
                        ...
                        className={`max-w-[220px] p-2.5 rounded-2xl text-xs leading-relaxed transition-all duration-200 ${
                          isUser
                            ? 'bg-ods-brown-dark text-white rounded-br-sm'
                            : 'bg-white border border-ods-brown-light/35 text-ods-brown-dark rounded-bl-sm shadow-sm'
                        } ${msg.risk ? 'cursor-pointer hover:scale-[1.02] hover:shadow-md active:scale-95' : ''} ${
                          isSelected ? 'ring-2 ring-offset-2 ring-ods-red-vibrant scale-[1.02] shadow-lg' : ''
                        }`}
                      >
                        {msg.text}
                      </div>
                      ...
                      {msg.risk && !isUser && (
                        <p className="text-[9px] text-ods-brown-medium/60 mt-0.5 ml-1">Toca para analizar</p>
                      )}
                    </div>
                  </div>
                )
            ...
            <div className="h-10 bg-ods-brown-dark" />
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col gap-4 h-full min-h-0" style={{ minWidth: 0 }}>
          ...
          <div className="bg-white border-2 border-ods-brown-light/30 rounded-2xl overflow-hidden shadow-lg flex-1 flex flex-col min-h-0">
            <div className="bg-gradient-to-r from-ods-brown-dark to-ods-brown-medium px-5 py-4 flex items-center gap-3 flex-shrink-0">
              ...
              <div>
                ...
                <p className="text-ods-beige/70 text-xs">Toca un globo del teléfono para ver el análisis</p>
              </div>
            </div>
            ...
            <div className="p-5 flex-1 overflow-y-auto">
              {!selectedMsg && !analyzing && (
                <div className="flex flex-col items-center justify-center h-full text-ods-brown-medium/40">
                  <AlertTriangle size={32} className="mb-2 opacity-35 animate-bounce" />
              ...
              {analyzing && (
                <div className="flex flex-col items-center justify-center h-full text-ods-brown-medium">
                  <Loader2 size={32} className="animate-spin mb-2 text-ods-orange" />
                  <p className="text-sm font-semibold">Analizando con IA experta...</p>
                </div>
              )}
              ...
              {!analyzing && selectedMsg && analysisResult && (() => {
                return (
                  <div className="animate-fadeIn">
                    <div className="rounded-xl border-2 p-4 bg-ods-beige/30 border-ods-brown-light/35 transition-all">
                      <div className="flex items-start justify-between gap-3 mb-3">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="text-sm font-extrabold text-ods-brown-dark">
                            Intención detectada: <span className="text-ods-red">{analysisResult.intention}</span>
                          </span>
                        </div>
                        <button 
                          onClick={() => { setSelectedMsg(null); setAnalysisResult(null) }}
                          className="text-ods-brown-medium hover:text-ods-brown-dark flex-shrink-0 cursor-pointer p-1 rounded-lg hover:bg-ods-beige/50"
                        >
                          <X size={16} />
                        </button>
                      </div>
                      ...
                      <div className="text-xs italic mb-3 px-3 py-2 rounded-lg bg-white border border-ods-brown-light/25 text-ods-brown-medium">
                        "{selectedMsg.text}"
                      </div>

                      {/* Explicación de la IA */}
                      <p className="text-sm text-ods-brown-dark leading-relaxed mb-4">{analysisResult.explanation}</p>

                      {/* Recomendación de acción */}
                      {analysisResult.recommendation && (
                        <div className="bg-white rounded-lg p-3 border border-ods-brown-light/20 shadow-sm">
                          <p className="text-xs font-extrabold text-ods-orange mb-1 flex items-center gap-1">
                            <span>💡</span> ¿Qué hacer?
                          </p>
                          <p className="text-sm text-ods-brown-dark leading-relaxed font-semibold">{analysisResult.recommendation}</p>
                        </div>
                      )}
                    </div>
                  </div>
          ...
          <div className="bg-white border-2 border-ods-brown-light/30 rounded-2xl p-4 shadow-sm flex-shrink-0">
            <h3 className="font-extrabold text-ods-brown-dark mb-3 text-xs uppercase tracking-wider flex items-center gap-2 select-none">
              <Sparkles size={14} className="text-ods-orange" /> Pregúntale a ROVIX
            </h3>
            <div className="grid grid-cols-2 gap-2">
              {quickQuestions.map((q, i) => (
                <button
                  ...
                  className="text-left p-2.5 bg-ods-beige/20 border border-ods-brown-light/30 rounded-lg text-xs text-ods-brown-dark hover:bg-ods-beige hover:border-ods-red transition-all disabled:opacity-50 flex items-center gap-2 group cursor-pointer"
                >
        ...
        <div className="w-[340px] flex-shrink-0 bg-white border-2 border-ods-brown-light/30 rounded-2xl overflow-hidden shadow-lg flex flex-col h-full">
          <div className="bg-gradient-to-r from-ods-brown-dark to-ods-brown-medium px-5 py-4 flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-white/15 flex items-center justify-center">
              <MessageSquareText size={18} className="text-white" />
            </div>
            <div className="flex-1">
              <h2 className="text-white font-bold">Chat con ROVIX</h2>
              <p className="text-ods-beige/70 text-xs">{chatLoading ? '● Escribiendo...' : '● En línea'}</p>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-3 bg-gradient-to-b from-ods-beige/30 to-white">
            {chatMessages.map(msg => (
              <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] ${msg.sender === 'user' ? '' : 'flex gap-2'}`}>
                  {msg.sender === 'ai' && (
                    <div className="w-6 h-6 rounded-lg bg-ods-red flex items-center justify-center flex-shrink-0 mt-1 shadow-sm">
                  ...
                  <div>
                    ...
                    <p className={`text-[10px] mt-1 ${msg.sender === 'user' ? 'text-right text-ods-brown-medium/50' : 'text-ods-brown-medium/50 ml-1'}`}>
            ...
            {chatLoading && (
              <div className="flex justify-start">
                <div className="flex gap-2">
                  ...
                  <div className="bg-white border border-ods-brown-light/35 rounded-2xl rounded-bl-sm p-2.5 shadow-sm">
                    <div className="flex items-center gap-1.5">
                      <div className="w-2 h-2 bg-ods-brown-medium rounded-full animate-bounce" style={{animationDelay: '0ms'}} />
                      <div className="w-2 h-2 bg-ods-brown-medium rounded-full animate-bounce" style={{animationDelay: '150ms'}} />
                      <div className="w-2 h-2 bg-ods-brown-medium rounded-full animate-bounce" style={{animationDelay: '300ms'}} />
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="p-3 bg-white border-t border-ods-brown-light/20 flex items-center gap-2">
            <input
              ref={inputRef}
              type="text"
              value={inputText}
              ...
              className="flex-1 bg-ods-beige rounded-xl px-3 py-2.5 text-xs outline-none focus:ring-2 focus:ring-ods-orange focus:bg-white transition-all border border-transparent text-ods-brown-dark"
              disabled={chatLoading}
            />
            <button
              ...
              className={`p-2.5 rounded-xl transition-all duration-200 cursor-pointer ${
                chatLoading || !inputText.trim()
                  ? 'bg-ods-beige text-ods-brown-medium/40 border border-transparent'
